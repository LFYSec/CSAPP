...
void Execve(char *filenamme, const char *argv[], const char *environ[]);
pid_t Fork();

/* Units defined by LFY */
void initsigproc(sigset_t *mask);
 ...
 * 
 * parseline -> check buildin_cmd -> block sigchld -> Fork() -> unblock sigchld -> exec
 *                                                           -> addjob -> unblok              
*/
void eval(char *cmdline)
{
...
void sigchld_handler(int sig)
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0){
        ...
        if(WIFSIGNALED(status)){
            printf("[%s] %s is terminated by signal.", pid2jid(pid), pid);
            deletejob(jobs, pid);
        }
...
void initsigproc(sigset_t *mask){
    ...
    // SIG_BLOCK set || oldset
    // SIG_UNBLOCK set && oldset
    // SIG_SETMASK set
...
void Execve(char *filenamme, const char *argv[], const char *environ[]){
    if(execve(filenamme, argv, environ) < 0){
